...
\textbf{\textit{Find:}}
\begin{itemize}
    \item The destruction of pipes must create a disconnected network (Preventing fuel from transporting across all bases).
...
\begin{itemize}
    ...
    \item \textbf{Equations (5) and (6)} if two connected bases $i$ and $j$ are assigned to different groups (i.e., $g_i \neq g_j$), then the pipeline connecting them must be destroyed ($d_e = 1$).
    \item \textbf{Equation (7)} guarantees that the partition is valid by ensuring neither group is empty.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Impact of Local Search (LS):} Incorporating neighborhood search significantly reduces the optimality gap compared to pure heuristics. GRASP + LS achieves the best performance among heuristic methods, while Greedy + LS yields competitive results in most instances, although it occasionally produces worse solutions than pure Greedy, this behavior can be attributed to the local search converging to a poor local optimum from a suboptimal initial solution.
