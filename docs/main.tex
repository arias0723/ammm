...
The Martian Network Destruction problem can be stated as follows:\\
\\
\textbf{\textit{Given}}:
\begin{itemize}
...
\textbf{\textit{Find:}} An assignment of specialists to pipes and a selection of pipes to destroy, \textbf{\textit{such that:}}
\begin{itemize}
    \item The destruction of the selected pipes creates a disconnected network (preventing fuel from being transported across all bases).

    \item Each specialist is assigned to at most one pipe.

    \item Each pipe selected for destruction receives enough specialist hours to meet its destruction requirement $t_{ij}$.
\end{itemize}
...
\noindent \textbf{Explanation of the equations:}
\begin{itemize}
    \item \textbf{Equation (1)} minimizes the total cost of all specialists assigned to destroy the pipelines.
    \item \textbf{Equation (2)} ensures that each specialist is assigned to at most one pipeline.
    \item \textbf{Equation (3)} guarantees that if a pipeline is selected to be destroyed ($d_e = 1$), the sum of working hours of the specialists assigned to it must meet or exceed the time required to destroy it ($t_{ij}$).
    \item \textbf{Equation (4)} ensures that a specialist can only be assigned to a pipeline if that pipeline is actually being destroyed.
    \item \textbf{Equations (5) and (6)} ensure that if two connected bases $i$ and $j$ are assigned to different groups (i.e., $g_i \neq g_j$), then the pipeline connecting them must be destroyed ($d_e = 1$).
    \item \textbf{Equation (7)} guarantees that the partition is non-trivial by ensuring neither group is empty.
\end{itemize}

\noindent Together, constraints (5)--(7) guarantee that the set of destroyed pipes forms a valid cut, separating the network into at least two non-empty components.
...
\begin{itemize}
    ...
    \item $Demand_p$: The demand in terms of hours for pipe $p$.
    \item $Partition$: Binary assignment of bases to groups $\{0, 1\}$. Equal to 0 if base $i$ belongs to group 0, 1 otherwise.
    \item $CutPipes$: Set of pipes crossing the partition (pipes $p = \langle i,j \rangle$ where $Partition[i] \neq Partition[j]$).
\end{itemize}

\textbf{Phase 2: Specialist Assignment Variables}
\begin{itemize}
    \item $Specialists$: Set of available specialists.
    \item $Capacity_s$: The capacity (hours) that specialist $s$ can provide.
...
\\[0.2cm]
We considered two algorithms to find the minimum cut partition of the network:

\begin{enumerate}
    \item \textbf{Karger's Algorithm} (randomized): Repeatedly contracts random edges weighted by demand until two super-nodes remain. The standard algorithm requires $O(n^2 \log n)$ iterations to guarantee finding the true minimum cut with high probability.

    \item \textbf{Stoer-Wagner Algorithm} (deterministic): Uses maximum adjacency ordering to systematically find the global minimum cut through a series of min-cut phases. This algorithm is better suited for dense graphs.
\end{enumerate}
...
We selected Karger's algorithm with \textbf{only one} iteration per Greedy exploration. This yields a randomized cut that introduces diversification, which is particularly valuable when the true minimum cut leads to an infeasible resource allocation.
\\[0.2cm]
...
\For{each $p \in SortedPipes$}
    ...
    \While{$RemainingDemand_p > 0$}
        ...
        \EndIf
        \Statex
        \State $Solution \gets Solution \cup \{\langle s, p \rangle\}$ \Comment{Assign specialist s to pipe p}
        \State $AssignedSpecialists \gets AssignedSpecialists \cup \{s\}$
        \State $RemainingDemand_p \gets RemainingDemand_p - Capacity_s$
    \EndWhile
    \Statex
    \If{\textbf{not} \Call{ValidateAssignment}{$Solution, p$}}  \Comment{Verify pipe p is covered}
...
\State $coveredCapacity \gets \sum_{s \text{ assigned to } p \text{ in } Solution} Capacity_s$
...
We define two complementary neighborhood moves for the \textbf{Phase~2} resource-allocation subproblem:
...
\State $incumbent \gets solution$
    ...
    \While{$time < endTime$ \textbf{and} $iter < maxIterations$}
        ...
        \EndIf
        \Statex
        \If{$incumbentCost$ unchanged this iteration} \textbf{break} \Comment{Local optimum reached}
        \EndIf
    \EndWhile
    \Statex
...
After construction, the candidate solution is improved using the Local Search method (Section~\ref{sec:localsearch}). The best solution over all GRASP iterations is returned.
...
\Function{GRASP}{$maxIterations, \alpha, localSearchConfig$}
    ...
    \State $Partition \gets \text{ComputeMinCut}()$ \Comment{Computed once}
    \State $CutPipes \gets \{p \in Pipes \mid Partition[base_i(p)] \neq Partition[base_j(p)]\}$
    \Statex
    \For{$iter = 1$ \textbf{to} $maxIterations$}
        \State $solution \gets \text{GraspConstruction}(\alpha, CutPipes, Specialists)$
        \Statex
...
We tested \bm{$\alpha$} from 0 to 1 in increments of 0.1, using datasets with 100 specialists, 100 bases, and 1000 pipes without conducting the local search procedure, and the maximum number of iterations for the constructive phase was set to 100.  For the results, we selected the best mean objective with the least standard deviation (risk-adjusted formula) over 100 runs per alpha value, thereby penalizing high variance and preferring consistent solutions. \\
\\
...
% \begin{tikzpicture}
% \begin{axis}[
%     title={\textbf{Quality of Solutions (10 instances of s100\_p1000)}},
...
% \edef\temp{\noexpand\addplot[green!70!black, thick, mark=triangle] coordinates {\greedydata};}\temp
% \edef\temp{\noexpand\addplot[yellow!80!black, thick, mark=square] coordinates {\graspdata};}\temp
% \edef\temp{\noexpand\addplot[blue, thick, mark=diamond] coordinates {\greedylsdata};}\temp
% \edef\temp{\noexpand\addplot[red, thick, mark=o] coordinates {\grasplsdata};}\temp
% \edef\temp{\noexpand\addplot[brown, thick, mark=star] coordinates {\cplexdata};}\temp

% \legend{Greedy, GRASP, Greedy + LS, GRASP + LS, ILP (Optimal)}
% \end{axis}
% \end{tikzpicture}
% \captionsetup{labelformat=empty}
% \caption{Figure 4.4: Solution Quality across 10 random instances of s100\_p1000}
% \label{fig:fig44}
% \end{figure}
...
\begin{itemize}
    \item \textbf{ILP Baseline:} The ILP model exhibits the highest computational burden, requiring up to 55 minutes to determine the global optimum for the largest instance. However, the ILP model's efficiency is highly sensitive to problem scale; for smaller instances (e.g., fewer than 100 pipes), it can solve to optimality within a practical timeframe, even faster than the heuristic approaches due to their fixed Python overhead.
    ...
    \begin{itemize}
        ...
        \item \textbf{GRASP + LS:} Demands higher computational effort, ranging between 4 and 70 seconds per instance (noting that a time limit of 60 seconds was enforced per run). This increased overhead is attributable to the 100 constructive iterations of the GRASP metaheuristic combined with the local search applied at each iteration.
    \end{itemize}
\end{itemize}
...
\\
The results demonstrate a clear stratification in solution quality among the tested approaches:
\begin{itemize}
    \item \textbf{Optimal Baseline:}  The exact ILP model establishes the true lower bound, consistently outperforming all heuristic methods with an average objective value of 1187 across all instances.
    \item \textbf{Heuristic Trajectory:} The pure Greedy approach yields the worst solution quality, generating the highest cost curve across the largest instances (peaking near 4816 in instance s100p750). Standalone GRASP appears to be slightly more effective on average.
    \item \textbf{Impact of Local Search (LS):} Incorporating neighborhood search significantly reduces the optimality gap compared to pure heuristics. GRASP + LS achieves the best performance among heuristic methods, while Greedy + LS yields competitive results in most instances. However, Greedy + LS occasionally produces worse solutions than pure Greedy; this occurs because the single random Karger cut may yield a poor partition, and the local search then converges to a suboptimal local optimum from that starting point.
    \item \textbf{Top Metaheuristic:} The GRASP + LS hybrid achieves the lowest average objective cost of 2016 among all approximate methods, proving to be the most effective heuristic configuration for minimizing total cost.
\end{itemize}
...
Initially, we had limited knowledge of advanced graph theory and were unfamiliar with concepts like minimum cuts, graph partitions, and network connectivity algorithms. We approached the algorithmic design hesitantly at first, but after receiving guidance from our professors and exploring algorithms like Stoer-Wagner and Karger's in more depth, a new mathematical perspective opened up for us. We were impressed by the efficiency and power of specialized graph algorithms.\\
Moreover, the optimization methods used in integer linear programming represent a significant shift in thinking. These declarative methods are incredibly elegant, and we naturally strive to pursue optimal solutions in theory. However, in practice, solving these large-scale combinatorial problems to optimality can be prohibitively costly.
Consequently, we must employ heuristic and metaheuristic methods as a necessary tradeoff for scalability. It is essential to formulate, experiment, and iterate in order to discover practical solutions.\\
