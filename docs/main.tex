...
\begin{algorithmic}
\Function{ConstructSolution}{$\alpha, CutPipes, Specialists$}
\State $Solution \gets \phi$
\State $SortedPipes \gets \text{Sort}(CutPipes, key=Demand, reverse=\text{True})$
\State $SortedSpecialists \gets \text{Sort}(Specialists, key=CostEffectiveness)$
\Statex
\For{\textbf{each} $p \in SortedPipes$}
    ...
    \While{$RemainingDemand_p > 0$}
        ...
        \State \textcolor{red}{$ce_{best} \gets \min(\{CostEffectiveness_s \mid s \in Available\})$}
        \State \textcolor{red}{$ce_{worst} \gets \max(\{CostEffectiveness_s \mid s \in Available\})$}
...
\subsection{Solving Time Performance}

To evaluate computational efficiency, the execution times of the proposed algorithms were evaluated across ten problem instances. \nameref{fig:fig42} illustrates the solving time (in seconds) on a logarithmic scale for the configuration where $S = 100$ and $P = 1000$.
\\
...
\begin{itemize}
    \item \textbf{ILP Baseline:} The ILP model exhibits the highest computational burden, consistently requiring between $10^3$ and $10^4$ seconds (approximately 16 to 166 minutes) to determine the global optimum. A notable peak occurs in Instance 3, where the execution time approaches nearly 10,000 seconds. However, the text notes that the ILP model's efficiency is highly sensitive to problem scale; for smaller instances (e.g., fewer than 100 pipes), it can solve to optimality within a practical timeframe.
    \item \textbf{Greedy and GRASP Heuristics:} In stark contrast, both the pure Greedy algorithm and the standalone GRASP execution operate below the $10^{-1}$ second threshold, completing in less than 500 milliseconds. This represents an acceleration factor of roughly four orders of magnitude ($\approx 10,000\times$) relative to the ILP model.
    ...
    \begin{itemize}
        \item \textbf{Greedy + LS:} Consistently converges within a narrow band between $0.4$ and $1.0$ seconds across all instances.
        \item \textbf{GRASP + LS:} Demands higher computational effort, fluctuating between 2 and 8 seconds per instance. This increased overhead is attributable to the 100 iterations prescribed by the GRASP metaheuristic before invoking the neighborhood search.
    \end{itemize}
\end{itemize}


% Build time coordinates from individual CSV files
\newcommand{\buildtimecoords}[2]{%
    \gdef#1{}%
    \pgfplotsforeachungrouped \i in {0,...,9} {%
        \pgfplotstableread[col sep=comma]{data/#2/s100_p1000_instance_\i.csv}\temptable
        \pgfplotstablegetelem{0}{mean_time}\of\temptable
        \ifx#1\empty
...
\buildtimecoords{\greedytimedata}{s100_p1000_greedy}
\buildtimecoords{\grasptimedata}{s100_p1000_grasp}
\buildtimecoords{\greedylstimedata}{s100_p1000_greedy_ls}
\buildtimecoords{\grasplstimedata}{s100_p1000_grasp_ls}
\buildtimecoords{\cplextimedata}{s100_p1000_cplex}
...
\addplot[fill=green, bar shift=-10pt] coordinates {\greedytimedata};
\addplot[fill=yellow, bar shift=-5pt] coordinates {\grasptimedata};
\addplot[fill=blue, bar shift=0pt] coordinates {\greedylstimedata};
\addplot[fill=red, bar shift=5pt] coordinates {\grasplstimedata};
\addplot[fill=brown, bar shift=10pt] coordinates {\cplextimedata};
...
\nameref{fig:fig43} evaluates the solution quality by comparing the mean objective cost obtained by each algorithm across ten problem instances ($S = 100, P = 1000$). As this is a minimization problem, lower objective values represent superior performance.
\\
The results demonstrate a clear stratification in the quality of the solution among the tested approaches:
\begin{itemize}
    \item \textbf{Optimal Baseline:}  The exact ILP model establishes the true lower bound, consistently outperforming all heuristic methods with objective values spanning roughly 2,800 to 4,200.
    \item \textbf{Heuristic Trajectory:} The pure Greedy approach yields the worst solution quality, frequently generating the highest cost curve across the instances (peaking near 7,600 in Instance 7).
    \item \textbf{Impact of Local Search (LS):} Incorporating neighborhood search drastically minimizes the optimality gap. Standalone GRASP (executed for 100 iterations) and the hybrid Greedy + LS variant exhibit highly competitive, intermediate performance.
    \item \textbf{Top Metaheuristic:} Ultimately, the GRASP + LS hybrid consistently achieves the lowest objective costs among all approximate methods, proving to be the most effective heuristic configuration for minimizing total cost.
\end{itemize}


\subsection{Experiment Conclusion}

If the problem scale is small (e.g., fewer than 100 pipes), we should use the exact ILP model whenever possible, as it guarantees finding the absolute optimal solution within a rational timeframe. However, for larger networks (more than 100 pipes) where the combinatorial explosion makes the ILP unfeasible, we need effective approximations to deliver reasonably good solutions quickly.
