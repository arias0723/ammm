    ...
    def generate(self):
        ...
        for i in range(numInstances):
            ...
            for b in range(1, numBases):
                ...
                other = random.randint(0, numBases - 1)
                while other == b or other == b - 1 or pipeHours[b][other] != -1:
                    other = random.randint(0, numBases - 1)
                value = random.randint(minHours, maxHours)
                pipeHours[b][other] = value
                pipeHours[other][b] = value

            for k in range(numBases):
                for r in range(k + 1, numBases):
                    if pipeHours[k][r] == -1 and random.random() < 0.3 :
                        value = random.randint(minHours, maxHours)
                        pipeHours[k][r] = value
                        pipeHours[r][k] = value
